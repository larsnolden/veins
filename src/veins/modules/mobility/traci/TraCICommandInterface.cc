...
/**
 * Command to resume from any stopAt command
 * Was not implemented yet...
 * Implemented for CPS CAD course
 */
void TraCICommandInterface::Vehicle::resume() {
    uint8_t variableId = CMD_RESUME;
    uint8_t variableType = TYPE_COMPOUND;
    int zero = 0;
    TraCIBuffer buf = connection->query(CMD_SET_VEHICLE_VARIABLE, TraCIBuffer() << variableId << nodeId << variableType << zero);
    ASSERT(buf.eof());
}
